...
2. Lock every item referenced by this trade's `trade_items` (`for update`) and refuse if any of them is no longer `'available'` — see "Concurrent completion" below (added Phase 9).
3. Set `status='traded'` on every one of those items.
4. Set this trade's `status='completed'`.
5. Find every _other_ trade still in `pending` or `accepted_by_responder` that references any of those same items (via `trade_items`), and set those to `cancelled` — they're negotiating over items that no longer exist to trade. In practice, `accept_trade`'s own conflict resolution (above) means there's rarely anything left here by the time a trade completes; this step stays as the final backstop for any leftover pending trades on items involved in the accepted trade in ways `accept_trade` didn't already resolve (e.g. an item offered, not requested, in another pending trade).
...
**Concurrent completion (Phase 9, `0010_complete_trade_item_guard.sql`).** `accept_trade`'s conflict check (above) only locks the one trade row being accepted, not the item(s) involved — two `accept_trade()` calls for two *different* pending trades on the same item, landing at the same instant, can both pass the "already committed elsewhere?" check before either commits, so both can end up `accepted_by_responder` on the same item. This is a narrow, genuinely-concurrent-only race, not something sequential clicking can trigger. It used to self-heal the moment either trade completed, since step 5 above cancels the other trade regardless of whether it's still `pending` or already `accepted_by_responder` — except `complete_trade` never re-checked that its own items were still `available` before marking them traded. Two initiators confirming completion on two trades sharing an item, at the same instant, can deadlock in Postgres (each transaction ends up needing a lock the other is holding); Postgres's deadlock detector correctly aborts one side — that part is the database working as intended — but the aborted user's trade rolls back to `accepted_by_responder`, unchanged, so simply retrying (a natural reaction to the error) used to sail through every guard and silently mark a *second* trade completed over an item the first completion had already traded. Step 2 closes this: locking the items first means a concurrent `complete_trade()` over an overlapping item set blocks until the first one finishes, and the availability check then means a stale retry fails loudly ("...another trade over the same item(s) was already completed.") instead of corrupting the data.
